	...
	class WpAutomaticFacebook extends wp_automatic {
		...
		function fb_get_post($camp) {
			...
			if ((wp_automatic_trim( $cg_fb_page_id ) != '')) {
				...
				if($cg_fb_from == 'events' &&   (    $cu_info['http_code'] == 404   || stristr($exec,'The page you requested cannot be displayed right now') || preg_match( '!<div class="\w{1,2} \w{1,2} \w{1,2} \w{1,2} \w{1,2}"><span class="\w{1,2}">!' , $exec ) ||   stristr( $exec, '"area error"' )   )         ){
